...
(struct ws (to-scene pos view keyset) #:transparent)

(define (explore-anim pict)
...
(define (on-frame state n t)
  (match-define (ws ws.to-scene ws.pos ws.view ws.keyset) state)
...
(define (on-draw state n t)
  (match-define (ws ws.to-scene ws.pos ws.view ws.keys) state)
  (match-define (linear right forward up) ws.view)
  (combine
   (ws.to-scene t)
   (sunlight forward)
...
(define (on-key state n t k)
  ...
  (cond 
    [(set-member? (set-union MOVE-KEYSET LOOK-KEYSET) k)
     (ws ws.to-scene ws.pos ws.view (set-add ws.keys k))]
    [else state]))
...
(define (on-release state n t k)
  ...
  (cond
    [(set-member? (set-union MOVE-KEYSET LOOK-KEYSET) k)
     (ws ws.to-scene ws.pos ws.view (set-remove ws.keys k))]
    [else state]))
