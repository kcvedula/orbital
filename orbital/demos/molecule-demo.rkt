...
(define naphthalene
  (let* ([ring1-atoms (for/list ([i (in-range 1 7)]) (string->symbol (format "C~a" i)))]
         [ring2-atoms (for/list ([i (in-range 7 11)]) (string->symbol (format "C~a" i)))]
         [m (mol (make-immutable-hash) empty)]
         [m (for/fold ([m m]) ([atom-id (append ring1-atoms ring2-atoms)])
              (set-atom m atom-id (atom 'C #f #f #f)))]
         [m (for/fold ([m m]) ([i (in-range (length ring1-atoms))])
...
(define steroid
  (let* ([m (mol (make-immutable-hash) empty)]
         [ring-a-atoms (for/list ([i (in-range 1 7)]) (string->symbol (format "A~a" i)))]
         [ring-b-atoms (for/list ([i (in-range 1 7)]) (string->symbol (format "B~a" i)))]
         [ring-c-atoms (for/list ([i (in-range 1 7)]) (string->symbol (format "C~a" i)))]
         [ring-d-atoms (for/list ([i (in-range 1 6)]) (string->symbol (format "D~a" i)))]
         [m (for/fold ([m m]) ([atom-id (append ring-a-atoms (list 'B1 'B2 'B3 'B4)
                                                (list 'C1 'C2 'C3 'C4)
                                                ring-d-atoms)])
              (set-atom m atom-id (atom 'C #f #f #f)))]
         [m (for/fold ([m m]) ([i (in-range (length ring-a-atoms))])
